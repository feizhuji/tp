...
type (
	...
	PostReconnectPlugin interface {
		PostReconnect(PreSession) *Rerror
	}
...
func (p *pluginSingleContainer) postReconnect(sess PreSession, addr net.Addr) {
	for _, plugin := range p.plugins {
		if _plugin, ok := plugin.(PostReconnectPlugin); ok {
			if err := _plugin.PostReconnect(sess); err != nil {
				Fatalf("[PostReconnectPlugin:%s] network:%s, addr:%s, error:%s", plugin.Name(), addr.Network(), addr.String(), err.ToError())
				return
			}
		}
	}
